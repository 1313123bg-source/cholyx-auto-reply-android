...
class ReplyAccessibilityService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        handler.postDelayed({
            try {
                ...
                if (isGenerating(currentText)) return@postDelayed
                val currentEdit = findEditable(current) ?: return@postDelayed
                val currentSend = findSend(current) ?: return@postDelayed

                val args = Bundle().apply {
                    putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, message)
                }
                if (!currentEdit.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT, args)) return@postDelayed
    ...
    private fun findSend(node: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        ...
        val matches = label.contains("send") || label.contains("изпрати") || label.contains("submit") || label.contains("arrow up")
